...
namespace d2dFramework
{
	...
	private:
		...
		std::unordered_map<unsigned int, std::unordered_set<unsigned int>> mOnCollisionAdjLists; // stay, exit 감지 로직 
		std::queue<std::pair<unsigned int, unsigned int>> mOnEnterCollisionQueue; // enter, enter 관련된 콜백 호출 후 stay로 이동
		std::queue<std::pair<unsigned int, unsigned int>> mOnExitCollisionQueue; // exit, exit 관련된 콜백 호출 후 삭제
